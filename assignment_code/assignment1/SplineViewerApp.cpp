...
void SplineViewerApp::LoadFile(const std::string& filename, SceneNode& root) {
  ...
  if (spline_type == "Bezier curve") {
    // spline basis
    SplineBasis basis = SplineBasis::Bezier;

    for (int i = 0; i < control_points.size() - 1; i+=3)
    {
      auto start = control_points.begin() + i;
      auto end = control_points.begin() + i + 4;

      std::vector<glm::vec3> ctrl_pts(4);

      copy(start, end, ctrl_pts.begin());

      if (control_points.size() == 4){
        root.AddChild(make_unique<CurveNode>(ctrl_pts, basis, true));
      }
      else {
        root.AddChild(make_unique<CurveNode>(ctrl_pts, basis, false));
      }
    }
  }
  
  if (spline_type == "B-Spline curve"){
    SplineBasis basis = SplineBasis::BSpline;
